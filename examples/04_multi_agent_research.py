...
|------|------|
| Session | `{session_id}` |
| 子任务 | {stats['total_count']} |
| 成功 | {stats['success_count']} |
| 执行轮次 | {stats['rounds']} |
| 工具调用 | {stats['total_tool_calls']} |
| 重规划 | {stats['total_replans']} |
| 总耗时 | {stats['total_elapsed']:.1f}s |
| 规划耗时 | {plan_elapsed:.1f}s |
| L2 摘要 | {memory.l2.count} 条 |

> Generated by HorizonRL-Agent v0.1.0
"""
    ...
    print_status("report", f"已保存: {report_path} ({len(report_text)} 字符)")
    print(f"\n{report_text[:2000]}")
