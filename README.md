...
**AI 深度研究助手 — 自动搜索 · 交叉验证 · 撰写报告**

[![Python](https://img.shields.io/badge/Python-3.10+-blue.svg)](https://www.python.org/)
[![License](https://img.shields.io/badge/License-MIT-green.svg)](LICENSE)
[![Tests](https://img.shields.io/badge/Tests-355%20passed-brightgreen.svg)](tests/)
...
HorizonRL-Agent 是一个 AI 深度研究助手。输入一个研究问题，它会自动搜索网络和学术论文、
交叉验证结果、撰写结构化报告。支持 CLI / Web / API 三种使用方式，可私有化部署。

**核心功能**: 层次化记忆 (L1→L2→L3) · 验证器引导重规划 · 异步多 Agent DAG 编排
...
| 06 | `06_ablation_study.py` | 组件分析 (5 配置 + 压力注入) | 否 |
...
| Web | FastAPI + SSE + Token 流式 |
| 测试 | pytest + pytest-asyncio (330 项, 11 个测试模块) |

## 产品特性

1. **层次化记忆**: L1 (工作) → L2 (语义) → L3 (情景), Embedding 向量检索
2. **验证器引导重规划**: 9 条诊断规则 → 定向局部补丁, 自动恢复
3. **轨迹级日志**: 30 种事件类型, per-node 追踪, 全链路可观测
4. **证据溯源链**: 报告中的每个结论可追溯至搜索来源
5. **5 种任务自动分类**: research / code / comparison / summary / factual_qa
6. **Web 实时交互**: FastAPI + SSE 流式推送 + Token 级写作预览
